...
const Stats = ({ dt }) => {
    ...
    const ItemCard = ({ item, index }) => {
        return (
            <Card className='flex items-center justify-between w-full h-48 gap-5 px-4 py-12 shadow-lg 2xl:px-8 dark:border'>
                <div className='flex items-center w-full h-full gap-4'>
                    <div className={`w-12 h-12 flex items-center justify-center rounded-full ${ICON_STYLE[index]}`}>
                        {item.icon}
                    </div>
