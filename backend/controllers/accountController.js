...
export const getAccounts = async (req, res) => {
    try {
        ...
        res.status(200).json({ data: accounts.rows, status: "success" });
...
export const createAccount = async (req, res) => {
    try {
        ...
        if (!name || !account_number || amount == null) {
      return res.status(400).json({
        status: "failed",
        message: "Name, account number, and amount are required",
      });
    }
        ...
        if (accountExist) {
            return res.status(400).json({ message: "Account already exists", status: "failed" });
        }
...
export const addMoneyToAccount = async (req, res) => {
    try {
       ...
       await pool.query(transactionQuery);
        ...
        res.status(200).json({ data: accountInformation, status: "success", message: "Account updated successfully" });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: error.message, status: "failed" });
    }
