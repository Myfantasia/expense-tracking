...
export const addTransaction = async (req, res) => {
    try {
        ...
        if(Number(amount) <= 0 )
            return res.status(400).json({ message: "Amount must be greater than zero", status: "failed" });
        ...
        if (accountInfo.account_balance <= 0 || accountInfo.account_balance < Number(amount)) {
            return res.status(400).json({ message: "Insufficient account balance", status: "failed" });
        }
    ...
    await pool.query({
      ...
      values: [userId, description, "expense", "Completed", amount, source],
        ...
        res.status(201).json({ message: "Transaction completed successfully", status: "success" });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: error.message, status: "failed" });
    }
...
export const transferMoneyToAccount = async (req, res) => {
    try {
        ...
        if (!fromAccount) {
            return res.status(404).json({ message: "From account not found", status: "failed" });
        }
        ...
        const description = `Transfer to account (${fromAccount.account_name} - ${toAccount.rows[0].account_number})`;
        ...
        const description1 = `Receive from account (${fromAccount.account_name} - ${toAccount.rows[0].account_number})`;
        ...
        res.status(200).json({ message: "Transfer completed successfully", status: "success" });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: error.message, status: "failed" });
    }
